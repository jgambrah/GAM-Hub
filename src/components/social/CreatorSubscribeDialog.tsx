...
import { 
    ...
    MessageSquare, Swords, Zap, Globe, Coins, CreditCard 
} from 'lucide-react';
...
const MONTHLY_PRICE_GHS = 30; // ~ $3.00
...
export function CreatorSubscribeDialog({ creator, isOpen, onClose }: CreatorSubscribeDialogProps) {
  ...
  const paystackConfig = {
    ...
    email: user?.email || '',
    amount: MONTHLY_PRICE_GHS * 100, // GHS to Pesewas
  ...
  const handleSubscribe = () => {
    ...
    initializeSubscription({
        onSuccess: async (ref: any) => {
            ...
            try {
                if (firestore && user) {
                    await subscribeToCreator(firestore, user.id, creator.id, MONTHLY_PRICE_GHS, ref.reference);
                    toast({
                        title: "Subscribed! 💎",
        ...
        <div className="p-10 space-y-8 bg-background">
          ...
          <div className="p-6 bg-slate-900 rounded-[2rem] border-2 border-amber-500/30 flex flex-col items-center text-center relative overflow-hidden">
             ...
             <div className="flex items-baseline gap-1 relative z-10">
                <span className="text-4xl font-black text-white italic tracking-tighter">GHS {MONTHLY_PRICE_GHS}</span>
