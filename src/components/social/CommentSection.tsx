...
export default function CommentSection({ postId }: { postId: string }) {
  ...
  return (
    <div className="mt-6 pt-6 border-t border-border">
      ...
      <form onSubmit={postComment} className="flex items-center gap-3 bg-muted p-2 rounded-[2rem]">
        <input 
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="Drop a comment..." 
          className="flex-1 bg-transparent border-none outline-none text-sm font-medium px-4"
          disabled={isPosting}
        />
        <button type="submit" disabled={!text.trim() || isPosting} className="p-3 bg-foreground text-background rounded-full">
