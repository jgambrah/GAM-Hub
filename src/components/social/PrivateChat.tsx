...
import { Send, Smile, Reply, Forward, X, ShieldCheck, Paperclip } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50 no-scrollbar">
          {messages?.map((msg: Message) => (
            <div key={msg.id} className={`flex ${msg.senderId === auth.currentUser?.uid ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] group relative p-4 text-sm font-medium leading-relaxed ${msg.senderId === auth.currentUser?.uid ? 'bg-blue-600 text-white rounded-3xl rounded-tr-none' : 'bg-white text-slate-800 rounded-3xl rounded-tl-none shadow-sm'}`}>
          ...
          <form onSubmit={handleSendMessage} className="flex items-center gap-2">
            <div className="relative flex-1">
              <input 
                ...
                className="w-full bg-slate-50 p-4 pl-20 rounded-[2rem] border-none outline-none text-sm font-medium"
              />
              <div className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center gap-1">
                <button type="button" onClick={() => setShowEmoji(!showEmoji)} className="p-2 text-slate-400 hover:text-blue-600 transition-colors">
                  <Smile size={22} />
                </button>
            ...
            <button type="submit" className="p-4 bg-blue-600 text-white rounded-full shadow-lg shadow-blue-100 active:scale-90 transition-transform">
              <Send size={20} />
            </button>
          </form>
        </div>
