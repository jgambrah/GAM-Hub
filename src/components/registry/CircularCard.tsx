...
export function CircularCard({ post }: { post: RegistryPost }) {
  ...
  return (
    <div className={cn(
        ...
        post.isUrgent ? 'border-red-200 dark:border-red-900 border-l-red-600 ring-4 ring-red-50 dark:ring-red-900/10' : 'border-slate-100 dark:border-slate-800 border-l-blue-900'
    )}>
      {/* Background Graphic */}
      <div className="absolute right-0 top-0 p-8 opacity-5 -mr-4 -mt-4 pointer-events-none group-hover:rotate-12 transition-transform duration-700">
        <ShieldCheck size={120} className={post.isUrgent ? 'text-red-600' : 'text-blue-900'} />
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div className="flex items-center gap-3">
          <div className={cn(
              ...
              post.isUrgent ? "bg-red-50 text-red-600 dark:bg-red-900/20" : "bg-blue-50 text-blue-900 dark:bg-blue-900/20"
          )}>
      ...
      {attachment && (
        <button 
          ...
          className={cn(
              ...
              post.isUrgent ? "bg-red-600 text-white hover:bg-red-700" : "bg-slate-900 text-white hover:bg-blue-900"
          )}
        >
          <Eye size={18} />
