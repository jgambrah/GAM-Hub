...
export function useVibeAds(activeMood: VibeMood) {
  ...
  // Rotates through eligible ads by slot index so different positions show
  // different ads rather than always repeating the top-ranked one.
  ...
  const recordImpression = useCallback((adId: string) => {
    ...
    addDoc(collection(firestore, 'ad_campaigns', adId, 'impressions'), {
      ...
      sessionId: (globalThis as any).__vibeSessionId ?? 'unknown',
    }).catch(() => {});
  ...
  }, [firestore, user?.id, user?.campusId]);
